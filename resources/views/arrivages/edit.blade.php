@extends('layouts.app')
@section('title', 'Modifier Arrivage ' . $arrivage->reference)
@section('page-title', 'Modifier Arrivage : ' . $arrivage->reference)
...
@section('content')
<form method="POST" action="{{ route('arrivages.update', $arrivage) }}" id="arrivageForm">
    @csrf
    @method('PUT')
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div id="produits-container">
                        @forelse($arrivage->produits as $index => $ligne)
                        <div class="produit-row flex-row-mobile" style="margin-bottom: 12px; border-bottom: 1px dashed var(--border); padding-bottom: 12px;">
                            <div style="flex: 2;">
                                ...
                                <div class="autocomplete-wrap">
                                    <input type="text" class="form-control produit-search" placeholder="Tapez le nom du produit..." data-row="{{ $index }}" autocomplete="off" value="{{ $ligne->produit?->nom }}" required>
                                    <input type="hidden" name="produits[{{ $index }}][produit_id]" class="produit-id" value="{{ $ligne->produit_id }}">
                                    <input type="hidden" name="produits[{{ $index }}][fournisseur_id]" class="fourn-id" value="{{ $ligne->fournisseur_id }}">
                                    <div class="autocomplete-dropdown" data-row="{{ $index }}"></div>
                                </div>
                            </div>
                            <div style="flex: 1;">
                        ...
                        <div style="display: flex; gap: 6px; align-items: center;">
                            <select name="magasin_id" id="magasin-select" class="form-control" required style="flex: 1;">
                                @foreach($magasins as $m)
                                    <option value="{{ $m->id }}" {{ $arrivage->magasin_id == $m->id ? 'selected' : '' }}>{{ $m->nom }}</option>
                                @endforeach
                            </select>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <button type="submit" class="btn btn-primary" style="width: 100%; justify-content: center; margin-top: 10px;">
                        <i class="bi bi-save"></i> Mettre à jour l'Arrivage
                    </button>
                    
                    <a href="{{ route('arrivages.show', $arrivage) }}" class="btn btn-secondary btn-sm" style="width: 100%; justify-content: center; margin-top: 8px;">
                        Annuler
                    </a>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let rowCount = {{ max($arrivage->produits->count(), 1) }};
        ...
        if (deviseSelect) deviseSelect.addEventListener('change', () => updateDevise(true));
        tauxInput.addEventListener('input', updateTaux);
        updateDevise(false);
        const noteInit = document.getElementById('taux-note');
        if (noteInit) noteInit.textContent = 'Taux enregistré — changez la devise pour le taux de marché en temps réel.';
        ...
        // ─── Initialiser autocomplete pour chaque ligne existante ──────
        container.querySelectorAll('.produit-search').forEach(input => {
            initAutocomplete(input);
        });

        // ─── Appliquer les couleurs/tags fournisseur aux lignes pré-remplies ──
        container.querySelectorAll('.produit-row').forEach(row => {
            const fournId = row.querySelector('.fourn-id').value;
            if (fournId) {
                const fourn = fournisseursData.find(f => String(f.id) === String(fournId));
                if (fourn) {
                    ...
                    if (label) {
                        ...
                        tag.innerHTML = `<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${color};"></span> ${fourn.nom}`;
                        label.after(tag);
                    }
