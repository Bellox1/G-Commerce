...
const CreateArrivageScreen = ({ navigation }) => {
    ...
    const [lignes, setLignes] = useState([
        { produit_id: null, fournisseur_id: null, quantite: '10', prix_unitaire_origine: '5000' }
    ]);
    ...
    const fetchFormData = async () => {
        try {
            ...
            if (prodList.length > 0) {
                setLignes([{ produit_id: null, quantite: '10', prix_unitaire_origine: '5000' }]);
            }
        } catch (e) {
            console.error('Error fetching data for arrivage:', e);
        } finally {
    ...
    const addLigne = () => {
        setLignes([...lignes, { produit_id: null, fournisseur_id: null, quantite: '1', prix_unitaire_origine: '0' }]);
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert('Succès', 'Nouvel arrivage enregistré avec succès !');
            navigation.goBack();
        } catch (e) {
            const msg = e.response?.data?.message || 'Erreur lors de la création de l\'arrivage';
            Alert.alert('Erreur', msg);
        } finally {
            setSubmitting(false);
        }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={[styles.topBar, { paddingTop: Math.max(insets.top, 16) }]}>
                ...
                <Text style={styles.topTitle}>Nouvel Arrivage</Text>
                <View style={{ width: 36 }} />
            </View>
            ...
            <KeyboardAwareScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit} disabled={submitting}>
                    {submitting ? (
                        ...
                        <Text style={styles.submitBtnText}>Enregistrer l'Arrivage</Text>
                    )}
                </TouchableOpacity>
...
export default CreateArrivageScreen;
